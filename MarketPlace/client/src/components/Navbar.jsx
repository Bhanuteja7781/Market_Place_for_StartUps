        ...
        <nav className='h-20'>
            <div className='fixed left-0 top-0 right-0 z-100 flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white transition-all'>
                ...
                <div className='hidden sm:flex items-center gap-4 md:gap-8 max-md:text-sm text-gray-800'>
                    ...
                    <Link onClick={() => scrollTo(0, 0)} to='/marketplace'> Startup Marketplace </Link>
                    {user ? <Link onClick={() => scrollTo(0, 0)} to='/messages'> Investor Messages </Link> : <Link onClick={openSignIn} to='#'> Messages </Link> }
                    {user ? <Link onClick={() => scrollTo(0, 0)} to='/my-listings'> My Startups </Link> : <Link onClick={openSignIn} to='#'> My Listings </Link> }
                </div>
