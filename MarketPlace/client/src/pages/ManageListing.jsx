    ...
    return (
        <div className='min-h-screen py-8'>
            <div className='max-w-4xl mx-auto px-4 sm:px-6 lg:px-8'>
                <div className='mb-8'>
                    <h1 className='text-3xl font-bold text-gray-800'>{isEditing ? 'Edit Startup' : 'List Your Startup'}</h1>
                    <p className='text-gray-600 mt-2'>{isEditing ? 'Update your startup listing' : 'Create a startup listing to showcase your company to investors'}</p>
                </div>

                <form onSubmit={handleSubmit} className='space-y-8'>
                    <Section title='Startup Information'>
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                            <InputField label='Startup Name *' value={formData.title} placeholder='e.g., AI-Powered Travel Platform' onChange={(v) => handleInputChange('title', v)} required={true} />
                            <SelectField label='Startup Type *' options={platforms} value={formData.platform} onChange={(v) => handleInputChange('platform', v)} required={true} />
                            <InputField label='Startup Identifier *' value={formData.username} placeholder='startup-name' onChange={(v) => handleInputChange('username', v)} required={true} />
                            <SelectField label='Industry / Sector *' options={niches} value={formData.niche} onChange={(v) => handleInputChange('niche', v)} required={true} />
                        </div>
                    </Section>

                    <Section title='Startup Metrics'>
                        <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'>
                            <InputField label='Interested Investors *' type='number' min={0} value={formData.followers_count} placeholder='100' onChange={(v) => handleInputChange('followers_count', v)} required={true} />
                            <InputField label='Investor Interest Rate (%)' type='number' min={0} max={100} placeholder='4' value={formData.engagement_rate} onChange={(v) => handleInputChange('engagement_rate', v)} />
                            <InputField label='Monthly Traction / Exposure' type='number' min={0} placeholder='10000' value={formData.monthly_views} onChange={(v) => handleInputChange('monthly_views', v)} />
                        </div>

                        <div className='grid grid-cols-1 md:grid-cols-2 gap-6 mb-6'>
                            <InputField label='Primary Market Country' value={formData.country} placeholder='United States' onChange={(v) => handleInputChange('country', v)} />
                            <SelectField label='Target Customer Age Range' options={ageRanges} value={formData.age_range} onChange={(v) => handleInputChange('age_range', v)} />
                        </div>

                        <div className='space-y-3'>
                            <CheckboxField label='Startup is legally registered' checked={formData.verified} onChange={(v) => handleInputChange('verified', v)} />
                            <CheckboxField label='Startup is generating revenue' checked={formData.monetized} onChange={(v) => handleInputChange('monetized', v)} />
                        </div>
                    </Section>

                    <Section title='Funding & Pitch Details'>
                        <InputField label='Funding Goal (USD) *' type='number' min={0} value={formData.price} placeholder='250000' onChange={(v) => handleInputChange('price', v)} required={true} />
                        <TextareaField label='Startup Pitch Description *' value={formData.description} onChange={(v) => handleInputChange('description', v)} required={true} />
                    </Section>

                    <Section title='Pitch Deck & Proof'>
                        <div className='border-2 border-dashed border-gray-300 rounded-lg p-6 text-center'>
                            ...
                            <p className='text-sm text-gray-500 mt-2'>Upload pitch deck, traction screenshots, or revenue proof</p>
                        </div>
                    </Section>
